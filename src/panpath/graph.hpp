...
struct Graph {
    ...
    std::shared_ptr<Spool> segment_spool, traversal_spool;
    std::map<std::string, std::vector<TraversalRecord>> records;
    std::set<std::string> path_names;
    std::string version = "inferred-1.x", compression;
    std::size_t path_record_count = 0, walk_record_count = 0;
    bool unspecified_overlaps_as_blunt = false;
};

struct GraphReadResult { Graph graph; std::vector<Diagnostic> diagnostics; };
